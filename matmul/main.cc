...
#ifdef COLUMN_MAJOR
#if defined(MKL) || defined(BLASFEO) || defined(OPENBLAS) || defined(BLIS) ||  \
    defined(ACCELERATE)
...
void matmul(float *aligned_a, float *allocated_a, int64_t offset_a,
            int64_t size_a0, int64_t size_a1, int64_t strides_a0,
            int64_t strides_a1, float *aligned_b, float *allocated_b,
            int64_t offset_b, int64_t size_b0, int64_t size_b1,
            int64_t strides_b0, int64_t strides_b1, float *aligned_c,
            float *allocated_c, int64_t offset_c, int64_t size_c0,
            int64_t size_c1, int64_t strides_c0, int64_t strides_c1);
}
#endif
...
tvm::runtime::Module create_module() {
  // Define algorithm
  tvm::te::Tensor A =
      tvm::te::placeholder({MDIM, KDIM}, tvm::DataType::Float(32), "A");
  tvm::te::Tensor B =
      tvm::te::placeholder({KDIM, NDIM}, tvm::DataType::Float(32), "B");
  auto k = tvm::te::reduce_axis(tvm::Range{0, KDIM}, "k");
  // TODO: Add column major support to TVM
  int bn = 32;
  auto packedB = tvm::te::compute(
      {NDIM / bn, KDIM, bn},
  ...
  auto C = tvm::te::compute(
      ...
      [&](tvm::te::Var i, tvm::te::Var j) {
        return tvm::sum(
            A[i][k] * packedB[tvm::floordiv(j, bn)][k][tvm::indexmod(j, bn)],
            {k});
      },
      "C");
  ...
  s[C].tile(cAxis[0], cAxis[1], bn, bn, &x_outer, &y_outer, &x_inner, &y_inner);
...
void naive_matmul(const float *a, const float *b, float *c, size_t m, size_t k,
                  size_t n) {
  // correctness check
...
static void BenchmarkFunction(benchmark::State &state) {
  ...
  CHECK_CUDA(
      cudaMemcpy(devA, A, MDIM * KDIM * sizeof(float), cudaMemcpyHostToDevice));
  CHECK_CUDA(
      cudaMemcpy(devB, B, KDIM * NDIM * sizeof(float), cudaMemcpyHostToDevice));
  CHECK_CUDA(
      cudaMemcpy(devC, C, MDIM * NDIM * sizeof(float), cudaMemcpyHostToDevice));
#endif
  ...
  tvm::runtime::Module module =
      tvm::runtime::Module::LoadFromFile(TO_STRING(TVM_LIB));
  ...
  for (auto _ : state) {
...
#elif defined(HALIDE)
#if defined(COLUMN_MAJOR)
    hblas_sgemm(MATRIX_FORMAT, HblasNoTrans, HblasNoTrans, MDIM, NDIM, KDIM,
                alpha, A, LDA, B, LDB, beta, C, LDC);
#else
    ...
    hblas_sgemm(MATRIX_FORMAT, HblasNoTrans, HblasNoTrans, NDIM, MDIM, KDIM,
    ...
    int n0 = NDIM;
    int k0 = KDIM;
    blas_sgemm(&c_t, &c_t, &m0, &n0, &k0, &alpha, A, &LDA, B, &LDB, &beta, C,
               &LDC);
#elif defined(ACCERA)
    PP_CONCAT(PP_CONCAT(PP_CONCAT(acc_matmul_, MDIM), PP_CONCAT(x, NDIM)),
              PP_CONCAT(x, KDIM))
    (A, B, C);
#elif defined(RUY)
...
#ifdef COLUMN_MAJOR
    matmul(A, A, 0, MDIM, KDIM, 1, LDA, B, B, 0, KDIM, NDIM, 1, LDB, C, C, 0,
           MDIM, NDIM, 1, LDC);
#else
    matmul(A, A, 0, MDIM, KDIM, LDA, 1, B, B, 0, KDIM, NDIM, LDB, 1, C, C, 0,
           MDIM, NDIM, LDC, 1);
#endif
#elif defined(NAIVE)
...
int main(int argc, char **argv) {
  ...
  ::benchmark::RegisterBenchmark(
      "BM_Matmul",
      [](benchmark::State &state) -> void { BenchmarkFunction(state); })
      ->MeasureProcessCPUTime()
      ->UseRealTime();
  //->Iterations(NUM_REPS); //A fixed number of iterations can be set by
  // uncomment this
